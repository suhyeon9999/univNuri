	...
	<div id="wrap">
		<div class="total-container">
			<div class="title-container">
				...
				<h1>개인정보 관리</h1>
				<button onclick="aMyPageUpdateOK()" id="btn-submit">저장</button>
				<button onclick="location.href='/aMyPageDetail'" id="btn-cancel">취소</button>
				<input type="hidden" name="" value="" />
			</div>
			<div class="main-container">
				<div id="profile">
					<c:if test="${not empty myPage.f_name}"><!-- 사진 올린 적 1번이라도 있는 사람 --> 
						<img
							src="${pageContext.request.contextPath}/resources/upload/${sessionScope.uvo.user_id}/${myPage.f_name}"
							alt="프로필 사진" />
							<!-- [경우의 수 1] 사진 2번 이상 올린적 있는 사람 (f_old_name이 NOT NULL) -->
							<c:if test="${not empty myPage.f_old_name }">
								<input type="hidden" id="f_old_name" name="f_old_name" value="${myPage.f_old_name}">
								<input type="hidden" id="f_curr_name" name="f_curr_name" value="${myPage.f_name}">
							</c:if>
							<!-- [경우의 수 2] 사진 딱 1번 올린 사람 (f_old_name이 NULL) -->
							<c:if test="${empty myPage.f_old_name }">
								<input type="hidden" id="f_old_name" name="f_old_name" value="null_data">
								<input type="hidden" id="f_curr_name" name="f_curr_name" value="${myPage.f_name}">
							</c:if>
					</c:if>
					<c:if test="${empty myPage.f_name}"> <!-- [경우의 수 3] 아예 사진 올린 적 없는 사람 -->
						<img
				...
				<div id="content">
					...
					<div class="input-form">
						<label for="phone">휴대폰</label> <input type="text"
							value="${myPage.phone }" id="phone" required>
					</div>
					<div class="input-form">
						<label for="email">E-mail</label> <input type="text"
							value="${myPage.email }" id="email" required>
					</div>
					<div class="input-form">
						<label for="birth">생년월일</label> <input type="date"
							value="${myPage.birth }" id="birth" required>
					</div>
