...
<body>
	...
	<div id="wrap">
		<form action="/enroll-apply-reservation-update-ok" method="post">
			<input type="hidden" value="${enroll_apply_idx}"
				name="enroll_apply_idx">
					...
					<div class="main-top-side">
						<div class="btn-group">
							<button type="button" class="btn-red"
								onclick="go_reservation_look()">취소</button>
						...
						<div class="count-wrap">
							...
							<p>
								선택된 강의 : <span id="selected-count">${lectListCheckCount}</span>
							</p>
						</div>
									...
									<c:forEach var="k" items="${lectList}" varStatus="v">
										<tr>
											<td><input type="checkbox" name="enrollCheckbox"
												value="${k.lect_idx}" data-subjects="${k.enroll}"
												<c:if test="${k.lect_active == 0}">checked</c:if>></td>
											<td>${v.index + 1}</td>
											<td>${k.lect_id}</td>
											<td>${k.lect_name}</td>
											<td>${k.dept_name}</td>
	...
	<script type="text/javascript">
		function go_reservation_look() {
			location.href = "/moveEnrollApplyList";
		}
	</script>
	...
	<script type="text/javascript">
		$(document)
				.ready(
						function() {
							// ✅ 1. 페이지 로딩 시 체크된 개수 표시
							function updateSelectedCount() {
								const checkedCount = $('input[name="enrollCheckbox"]:checked').length;
								$('#selected-count').text(checkedCount);
							}

							updateSelectedCount(); // 페이지 로딩 시 초기 개수 반영

							// ✅ 2. 체크 상태 변경 시 반영
							$('input[name="enrollCheckbox"]').on('change',
									function() {
										updateSelectedCount();
									});
						});
